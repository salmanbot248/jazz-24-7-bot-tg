...
def tmdb_info(query):
    try:
        ...
        r = requests.get(f"{base}/search/multi", params={
        ...
        mid = res["id"]
        if mtype == "tv":
            d = requests.get(f"{base}/tv/{mid}", params={"api_key": TMDB_API_KEY}, timeout=10).json()
            title = d.get("name","N/A")
            year  = d.get("first_air_date","")[:4]
            rt    = d.get("episode_run_time",[])
            runtime = f"{rt[0]} min/ep" if rt else "N/A"
            extra = f"📺 Seasons: {d.get('number_of_seasons','N/A')} | Episodes: {d.get('number_of_episodes','N/A')}"
        else:
            d = requests.get(f"{base}/movie/{mid}", params={"api_key": TMDB_API_KEY}, timeout=10).json()
        ...
        if len(overview) > 350: overview = overview[:350] + "..."
        ...
        text = (
            ...
            f"🌍 Country: {countries or 'N/A'}\n"
            f"🗣️ Language: {languages or 'N/A'}\n"
            f"⏱️ Duration: {runtime}\n"
